...
- **Product bar:** follow `PRODUCT-BAR.md`. Session is incomplete if only 14/14 / units moved and the Tip clip was not Mac-measured for **ordered advance** (or the Tip’s stated failure mode).
- **One concern** per session (Tip-named). Offline ONNX; no suite-ID hardcodes; no liturgy retune; no label-fill work.
