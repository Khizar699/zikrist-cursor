...
npm run test:replay -- liturgy             # salah liturgy stubs; skip missing_fixture (not PASS)
npm run test:replay -- salah-liturgy       # alias of liturgy
npm run test:replay -- --include-pending   # 14 + real-imam + liturgy stubs (stubs skip until WAV exists)
```
...
Salah liturgy replay is a **separate** pending pack (`prompts/salah-liturgy/manifest.stub.json`). Staging (gitignored): `artifacts/recitation/liturgy/<suite-id>/`. `npm run test:replay -- liturgy` (or `salah-liturgy`) lists/runs eight stubs and **skips** with `missing_fixture` — not PASS. No liturgy evaluation audio is committed; do not drop silent fake WAVs that would PASS. Mixed suites (`liturgy-then-fatiha`, `fatiha-then-takbeer`) may reuse EveryAyah Fatiha WAVs only for the Quran half after `npm run fixtures:recitation`; liturgy clips still skip until present. When audio exists later, scoring uses PCM through `RecitationFollower.lastHeardTokens` + `SalahLiturgyMatcher` (same as live listening), not token-fixtures that could PASS without audio. Sample skip JSON: `fixtures/salah-liturgy/sample-skip.json`.

Phrases still lacking audio (no WAV under `artifacts/recitation/liturgy/`):

- Suites expecting locks: `takbeer`, `thana`, `ruku_tasbih`, `sujood_tasbih`, `tashahhud` (plus mixed takbeer/thana around Fatiha).
