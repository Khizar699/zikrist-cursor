...
An offline Quran **mushaf follower** for iOS and Android: listen, lock the recited ayah, display its Arabic, and follow. This is a technical evaluation build, not a finished production application.

**Current MVP:** hear recitation, lock `[surah:ayah]`, paint that ayah’s Arabic, and follow. Translation is a later `[surah:ayah]` lookup — not current work.

**Terminal commands in order:** see **[SETUP.md](SETUP.md)**.
...
Evaluation audio (gitignored, not LFS):
...
The app is a single **mushaf listening** screen: microphone capture, on-device Tilawa / ONNX recognition, and an Arabic-only passage (`SHOW_TRANSLATION_PANE = false`). After the first lock, surrounding ayahs are already on screen; the focused ayah is full opacity and neighbors stay dim. It does not show a Quran translation, language picker, session history, accounts, cloud sync, analytics, training uploads, stories, chat, or local audio saving. Listening does **not** wait on an English/Urdu pack download (`MUSHAF_ONLY_MVP`).

An offline **salah liturgy phrase pack** (`assets/content/salah-liturgy.json`) ships Arabic plus pack-internal English glosses (liturgy/prayer gloss, not a Quran translation) for Phase 1 formulas (takbeer, thana, istiʿadha, ruku/sujood tasbih, rising from ruku, tashahhud, darood Ibrahimiyyah, amin, tasleem). A separate matcher (`src/core/salah-liturgy-matcher.ts`) scores Tilawa transcript tokens against that pack and emits `kind: 'salah_liturgy'` locks. The listening screen shows that phrase’s **Arabic** (English gloss is not painted in this mushaf-first slice). Quran mushaf passage follow still takes over on the next `verse_match`. This v1 set is a common mosque subset, not every madhhab variant, and is not claimed imam-ready. Qunoot and a separate liturgy Basmala row stay deferred. Verify the pack with `npm run liturgy:verify` (also hooked from `npm run assets:verify`). Headless liturgy replay is registered (`npm run test:replay -- liturgy`); `liturgy-takbeer` and `liturgy-thana` are `status: ready` after generating their gitignored WAVs (`npm run liturgy:tts -- liturgy-takbeer` / `liturgy-thana`) and remaining suites skip with `missing_fixture` (not a PASS). Quran replay `npm run test:replay -- all` remains the 14-suite regression gate. Matcher thresholds were not retuned for this harness pass.
...
Friends restoring **evaluation audio** only: `npm run fixtures:recitation` then `npm run fixtures:imam`. Liturgy TTS wavs: ffmpeg on `PATH` then `npm run liturgy:tts -- <id> --engine say` (Mac Majed).

The model and recognition tables are approximately 104 MB on disk, before native runtime and application overhead. They are excluded from Git and restored from pinned URLs with SHA-256 verification. The application bundles these assets. **This mushaf-first MVP does not auto-download a translation pack.** Internet is needed for development-server loading; a release build performs recognition without a server.

To test without Metro, build a release configuration, then disconnect networking:
...
1. There is no language picker and no translation download in this build. Canonical Arabic is bundled.
...
`npm test` is the **in-memory Phase 1 suite**: monotonic follow, surah-boundary ayah-1 handoff, madd/CTC cousins, bounded capture. **Do not add** network, SQLite translation schema, or multi-language pack download/activate tests to that command. Parked translation-file hashes stay on `npm run assets:verify`. Acoustic replay is `npm run test:replay`, not `npm test`.
...
To add a liturgy phrase later: append a row in `assets/content/salah-liturgy.json` with the eight required fields; set `arabic_recognition_normalized` via `normalizeLiturgyArabic`; set `sha256` to SHA-256 of `id`, `category`, `arabic_uthmani`, `arabic_recognition_normalized`, `english`, `source_note`, and `license_status` joined by newlines; then run `npm run liturgy:verify`. Do not put contested qunoot in the live list. Liturgy replay: generate takbeer/thana WAV on Mac with `npm run liturgy:tts -- liturgy-takbeer` or `npm run liturgy:tts -- liturgy-thana --engine say` (founder Mac: edge-tts 403; `say` Majed `ar_001`; ffmpeg on PATH e.g. `/tmp/ffmpeg-static`; same dest as clip_path), then `npm run test:replay -- liturgy-takbeer` / `liturgy-thana` (ready path; not skip). Other liturgy suites still skip until filled.

The tests cover serialized/bounded capture processing, acquire/follow/reacquire verse tracking (monotonic same-surah advance, Fatiha→body and last-ayah→ayah-1 handoffs, madd/CTC letter runs that must not Global Search, phonetic cousins that keep the lock, Al-Fatihah locking from 1:2, Falaq after 1:7, An-Nas staying off a long unrelated ayah, Al-Fatihah not locking Ibrahim 14:39/14:40 from shared Alhamdulillah, Kawthar then a later short surah, leftover Quraysh 106:1 after Asr 103:3, leftover Basmala after that last ayah not locking mushaf-next, a salah prior that cannot override a clearer acoustic match, Ikhlas not first-locking a long `قُلْ` lookalike such as 10:16, An-Nisa 4:129 not first-locking Fussilat 41:34 from a shared `ول-/تست-` prefix, and Ya-Sin 36:16 not first-locking short An-Naba 78:4 from a shared `علم` root), salah liturgy token locks versus Quran-like negatives, live first-match display versus silence-flush/jump guards, sequential helpers, held-verse display, and confirmed-match ordering. Default `npm test` does not open a translation SQLite or download a language pack. A separate in-memory pack-eviction policy unit exists for parked Phase 2; do not grow it into network/SQLite pack tests. Tilawa's patched search stays equivalent to the original algorithm, including a cap so short queries do not score all 6,236 verses.
...
- `src/core/salah-prior.ts`: editable salah **surah ranking** file (not liturgy text). Tie-break only; predictions are not displayed ayahs.
...
- `src/core/salah-liturgy-display.ts`: pack-row lookup for on-screen Arabic. Scores are not shown as percent certainty. Unknown ids show nothing. Quran translation is deferred.
...
- `src/services/content.ts`: immutable Arabic display and verse lookup from the bundled mushaf. Translation download/activate is parked (`MUSHAF_ONLY_MVP`); not a listen gate.
- `src/services/storage.ts`: local settings and installed pack records (packs unused while mushaf-only).
- `src/App.tsx`, `src/ui/`: single mushaf listening screen, Arabic verse list, Prayer / liturgy Arabic for a locked salah phrase, growing heard-word prefix until confirm, morphing listen control.
...
Read [AGENTS.md](AGENTS.md) for the product goal.
...
npm run fixtures:imam              # GitHub Release zip → artifacts/recitation/imam/ (see SETUP.md)
...
npm run test:bakeoff                       # Phase C: Tilawa follow clocks + skip rate (does not swap default)
npm run test:replay -- --engine tilawa     # same engine id; other ids exit blocked
```

Each suite writes `artifacts/qa-runs/replay-<suite>.json` with `matches[{surah,ayah,audioSeconds,score}]`, `firstLockSeconds`, `clocks`, `bakeoff` (five clocks + skip rate), `skips`, `failureMode`, `wrongSurahRate`, `wrongSurahCount`, and `firstLockWrongSurah`. `english-negative` PASSes only when no verse commits. `basmala-hold` PASSes only when `001001` alone locks neither 1:1 nor any other verse. Honest `failureMode` strings are expected when the current follower misses a suite — this harness does not retune acquire/follow.

Default `npm run test:replay -- all` runs the original **14** EveryAyah suites. Ready mosque suites: **`imam-mid-surah-cold`** (Dr Subayyal An-Nisa **4:129–130**) and **`imam-mid-surah-cold-qiyam`** (Qiyam-ul-Lail Faisal Ya-Sin **36:16–18**). Other real-imam suites stay `stub`. Founder-verified labels: `prompts/real-imam/LABELS.md` and `labels.json`. Restore clips with `npm run fixtures:imam` from GitHub Release tag `imam-fixtures-v1` (not git LFS; see [SETUP.md](SETUP.md)). Staging (gitignored): `artifacts/recitation/imam/<suite-id>/<qari-or-source>/`. `npm run test:replay -- real-imam` scores ready suites when the WAVs exist and **skips** remaining stubs with `missing_fixture`.
