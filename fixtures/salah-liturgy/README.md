...
`npm run test:replay -- liturgy` (alias `salah-liturgy`) **skips** remaining stub suites with `missing_fixture` (not a PASS). `liturgy-takbeer` and `liturgy-thana` are ready: generate WAV on Mac with `npm run liturgy:tts -- liturgy-takbeer` or `npm run liturgy:tts -- liturgy-thana` (`--engine say` if edge-tts 403), then those suites must PASS. Default `all` stays the original 14 Quran suites.
