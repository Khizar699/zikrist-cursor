...
if (cli.checkFixtures) {
  ...
  const restore = liturgyOnly
    ? 'Ready: npm run liturgy:tts -- liturgy-takbeer or liturgy-thana (Mac; --engine say if edge-tts 403). Stubs skip missing_fixture until their fill session. Mixed suites reuse EveryAyah Fatiha WAVs from artifacts/recitation/ after npm run fixtures:recitation. Do not invent silent WAVs.'
    : imamOnly
