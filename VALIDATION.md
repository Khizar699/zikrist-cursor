...
- Back-to-back reacquire: last ayah leftover is distinctive unexplained tokens (not a 100%-clean suffix, and not short crumbs such as `وال` that relatedStem to 103:1). Branch rebased onto current `main` `82d4f28` (Nas 114:6 `802bc66` included). Post-rebase Linux/x64 ONNX: `back-to-back` PASS 103:1@1 → 2@8.75 → 3@9.25 then 106:1@21.75 → 2@27.25 → 3@31.25 → 4@40.5 (`failureMode` null); standalone `asr` does not re-lock 103:1; `quraysh`, `fatiha`, `nas` (114:6@33.75), `kawthar`, `ikhlas`, `falaq`, `basmala-hold` PASS. Mac `npm run test:replay -- back-to-back` remains the merge gate and still needs a local re-run. Nas last-ayah window and the 55:1 Basmala-echo guard stay as on main. Not a physical-device claim.
...
| `back-to-back` | unit leftover crumbs from `11759a9` kept; ONNX not re-run here |

`npm test` 138/138 after rebase onto `11759a9`. Linux longer 2:1 first and nas/fatiha/short-surah Linux times above are from the **pre-rebase** tip. This workspace has no ONNX/WAV fixtures. Not a physical-device accuracy claim. **Do not merge until Mac `test:replay -- longer` first-locks 2:1**, and Mac nas/fatiha/asr/kawthar/quraysh stay green.
