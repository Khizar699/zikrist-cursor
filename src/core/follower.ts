...
/** Shared Basmala audio must not name the next surah from the post-surah pool. */
function isBasmalaOnlyWindow(recognized: string[]): boolean {
  if (!recognized.length) return true;
  const lead = leadingBasmalaWords(recognized);
  return lead > 0 && lead >= recognized.length;
}
...
/** Tokens after the current ayah, including when only its tail remains in the
 * follow window (the opening already slid out). Mixed leftover must not veto
 * the next surah’s ayah-1 opening. A shared one-token opening such as إنّا /
 * إنَّ must not be stripped from a same-surah repeat. */
function leftoverAfterHeard(recognized: string[], verseWords: string[]): string[] {
  const aligned = alignWordPositions(recognized, verseWords);
  const leftover = remainingAfterCurrent(recognized, verseWords);
  if (leftover.length < recognized.length && aligned.length >= 2) return leftover;
  if (aligned.length && aligned[0]!.spoken === 0 && aligned[0]!.verse > 0) {
    const tail = recognized.slice(aligned.at(-1)!.spoken + 1);
    if (tail.length) return tail;
  }
  let start = 0;
  while (start < recognized.length) {
    const word = recognized[start]!;
    const index = verseWords.findIndex((token) => wordsMatch(word, token) || relatedStem(word, token));
    if (index <= 0) break;
    start += 1;
  }
  return start > 0 && start < recognized.length ? recognized.slice(start) : recognized;
}
...
export class RecitationFollower {
  ...
  /** After the last ayah of a surah, prefer ayah 1 of the next located surah. */
  private preferSurahOpening = false;
  private wordIndex = -1;
  ...
  async feed(samples: Float32Array, timings: FeedTimings = {}): Promise<RecognitionMessage[]> {
    ...
    // After the last ayah, grow like acquire (up to 4 s). While waiting for a
    // short last ayah of this surah (114:6), keep the 5 s accumulate window.
  ...
  private followMaxSec(): number {
    ...
    if (this.preferSurahOpening) return ACQUIRE_MAX_SEC;
  ...
  private async follow(): Promise<RecognitionMessage[]> {
    ...
    const finishedSurah = atSurahBoundary || Boolean(alreadyComplete && !next);
    if (finishedSurah) this.preferSurahOpening = true;
    this.fresh = 0;
    const locate = this.mismatches > 0 || this.preferSurahOpening;
    ...
    // A completed last ayah must not hold the neighborhood through the next
    // surah (هو in 108:3 and 112:1 kept Kawthar locked until 112:4 on Mac).
    const neighborhood = finishedSurah
    ...
    const after = this.queryAfterCurrent(recognized, current);
    if (atSurahBoundary) {
      const pooled = this.poolAfterFinishedAyah(after, text, recognized, current);
      if (pooled) {
        const heard = after.recognized.length ? after.recognized : recognized;
        const pooledText = after.recognized.length ? after.text : text;
        return [...messages, ...this.commit(pooled, this.locationScore(pooledText, pooled), this.alignForCommit(heard, pooled))];
      }
    }

    // Last-ayah-only audio must not locate (112:4 لم يكن ≠ 98:1). New leftover speech may.
    const locateQuery = atSurahBoundary
      ? (after.recognized.length ? after : { recognized: [], text: '' })
      : { recognized, text };
    const located = locate || atSurahBoundary ? this.matchFromTranscript(result, atSurahBoundary && !locate) : null;
    const locatedVerse = located ? this.locateAyah(located, locateQuery.text, locateQuery.recognized) : undefined;
    const locatedScore = locatedVerse ? this.locationScore(locateQuery.text, locatedVerse) : 0;
    const stillInSurah = Boolean(next && next.surah === current.surah);
    const jump = Boolean(
      located
      && locatedVerse
      && !this.ambiguousSurah(located, locatedVerse, locateQuery.text)
      && !this.sameRef(locatedVerse, current)
      && !this.sameRef(locatedVerse, next)
      && neighborhood < NEIGHBORHOOD_KEEP
      && heardDistinct(locateQuery.recognized, locatedVerse, this.distinctSkip(locatedVerse))
      ...
      && this.canLock(located, locatedVerse, locateQuery.text, locateQuery.recognized)
      && (!this.preferSurahOpening || this.nextSurahUnusedEvidence(locatedVerse, locateQuery.recognized, current))
    );

    if (jump && located && locatedVerse) {
      const chosen = this.openingOfNewSurah(locatedVerse, locateQuery.text, locateQuery.recognized, current) ?? locatedVerse;
      if (!this.shouldHoldForSurahOpening(chosen, current, locateQuery.text, locateQuery.recognized)) {
        const alignFrom = locateQuery.recognized !== recognized
          && this.alignForCommit(locateQuery.recognized, chosen).length
          ? locateQuery.recognized
          : recognized;
        return [...messages, ...this.commit(chosen, located.score, this.alignForCommit(alignFrom, chosen))];
      }
    }
    ...
    if ((!advanced || sharedPrefixOnly) && neighborhood < NEIGHBORHOOD_KEEP) {
      // Waiting for the next surah is not tracking loss — do not reacquire
      // (and wipe) during leftover last-ayah audio before 112:1 starts.
      if (finishedSurah || this.preferSurahOpening) {
        this.mismatches = 0;
        return messages;
      }
      this.mismatches++;
  ...
  private alignForCommit(recognized: string[], verse: QuranVerse): number[] {
    ...
    const { words, basmala } = verseAlignWords(verse);
    const hits = new Set(aligned);
    for (let index = 0; index < words.length; index++) {
      const token = words[index]!;
      if (recognized.some((word) => wordsMatch(word, token) || softTokenMatch(word, token))) {
        hits.add(index + basmala);
      }
    }
    const ordered = [...hits].sort((left, right) => left - right);
    const skip = openingBasmalaWordCount(verse);
    const uniqueHits = ordered.filter((index) => index > skip);
    if (aligned.length && uniqueHits.length) return ordered.filter((index) => index >= skip);
    return uniqueHits.length ? uniqueHits : ordered;
  }

  private commit(verse: QuranVerse, score: number, matched: number[]): RecognitionMessage[] {
    const same = this.sameRef(verse, this.lock ?? undefined);
    if (same) {
      // Re-hearing the last ayah must not wipe the growing post-surah acquire
      // window (Mac jump needs ~4 s of Ikhlas, same as standalone).
      this.mismatches = 0;
      this.phase = 'following';
      if (matched.length) this.wordIndex = matched[matched.length - 1]!;
      const messages: RecognitionMessage[] = [];
      if (this.wordIndex >= 0) {
        messages.push({
          type: 'word_progress',
    ...
    const next = this.db.getNextVerse(verse.surah, verse.ayah);
    const finishedSurah = !next || next.surah !== verse.surah;
    this.preferSurahOpening = finishedSurah;
    this.phase = 'following';
    this.mismatches = 0;
    this.wordIndex = matched.length ? matched[matched.length - 1]! : -1;
    this.trimmedForShortLast = false;
    this.window = finishedSurah ? new Float32Array(0) : keepLast(this.window, KEEP_AFTER_COMMIT_SEC);
    this.fresh = 0;
  ...
  private bodyWords(verse: QuranVerse): string[] {
    return verseAlignWords(verse).words;
  }
  ...
  private uniqueOpeningSkip(verse: QuranVerse): number {
    const { words, basmala } = verseAlignWords(verse);
    if (words.length <= 1) return basmala;
  ...
  private locateAyah(match: QuranChampionMatch, text: string, recognized: string[]): QuranVerse | undefined {
    const span = this.ayahInSpan(match, text, recognized, this.preferSurahOpening || !this.closeRival(match));
  ...
  private ayahInSpan(
    ...
    const startAyah = this.preferSurahOpening ? 1 : match.ayah;
    let best: QuranVerse | undefined;
    let bestScore = -1;
    for (let ayah = startAyah; ayah <= end; ayah++) {
  ...
  /** Opening ASR often drops قل in a mixed post-surah window; unique body
   * tokens (هو الله أحد) are enough to name ayah 1 of the next short surah. */
  private surahOpeningHeard(verse: QuranVerse, recognized: string[], text: string): boolean {
    if (heardDistinct(recognized, verse, this.distinctSkip(verse)) && this.locationScore(text, verse) >= LOCK_SCORE) {
      return true;
    }
    if (this.hasVerseEvidence(text, verse, recognized)) return true;
    const { words, basmala } = verseAlignWords(verse);
    const from = Math.max(1, this.uniqueOpeningSkip(verse) - basmala);
    const distinctive = words
      .slice(from)
      .filter((token) => token.length >= 3 && !isAmbiguousAdvanceOpening(token));
    if (!distinctive.length) return false;
    const hits = distinctive.filter((token) => (
      recognized.some((word) => wordsMatch(word, token) || relatedStem(word, token) || softTokenMatch(word, token))
    ));
    return hits.length >= 2;
  }

  private scanPoolOpenings(surah: number, recognized: string[], text: string): QuranVerse[] {
    if (isBasmalaOnlyWindow(recognized)) return [];
    ...
    if (!verse || skipUnusableLock(verse) || !this.surahOpeningHeard(verse, recognized, text)) return [];
    return [verse];
  }
  ...
  private lockFromNextSurahPool(text: string, recognized: string[], fromSurah: number): QuranVerse | undefined {
    ...
    for (const surah of remainingAfter(fromSurah)) {
      for (const verse of this.scanPoolOpenings(surah, recognized, text)) {
        ...
        if (!this.surahOpeningHeard(verse, recognized, text)) continue;
        if (this.lock && !this.nextSurahUnusedEvidence(verse, recognized, this.lock)) continue;
        const score = this.locationScore(text, verse);
        pool.push({
          verse,
          score: Math.max(score, LOCK_SCORE),
  ...
  private queryAfterCurrent(recognized: string[], current: QuranVerse): { recognized: string[]; text: string } {
    const leftover = leftoverAfterHeard(recognized, verseAlignWords(current).words);
    if (!leftover.length) return { recognized: [], text: '' };
    const currentBody = verseAlignWords(current).words;
    const onlyCurrent = leftover.every((token) => currentBody.some((word) => (
      wordsMatch(word, token) || relatedStem(word, token) || softTokenMatch(word, token)
    )));
    if (onlyCurrent) return { recognized: [], text: '' };
    return { recognized: leftover, text: leftover.join(' ') };
  }

  /** Do not score a finished ayah’s own tokens as the next surah (112:4 لم يكن ≠ 98:1). */
  private poolAfterFinishedAyah(
    after: { recognized: string[]; text: string },
    text: string,
    recognized: string[],
    current: QuranVerse,
  ): QuranVerse | undefined {
    const fromLeftover = after.recognized.length
      ? this.lockFromNextSurahPool(after.text, after.recognized, current.surah)
      : undefined;
    if (fromLeftover) return fromLeftover;
    // Mixed leftover already tried. Last-ayah-only audio must not pool.
    if (!after.recognized.length) return undefined;
    if (after.recognized.join(' ') === recognized.join(' ')) return undefined;
    return this.lockFromNextSurahPool(text, recognized, current.surah);
  }

  /** Next-surah evidence must include a token the finished ayah does not use. */
  private nextSurahUnusedEvidence(verse: QuranVerse, recognized: string[], current: QuranVerse): boolean {
    const currentBody = verseAlignWords(current).words;
    const unused = (token: string) => !currentBody.some((word) => (
      wordsMatch(word, token) || relatedStem(word, token) || softTokenMatch(word, token)
    ));
    const { words, basmala } = verseAlignWords(verse);
    const from = Math.max(0, this.uniqueOpeningSkip(verse) - basmala);
    const distinctive = words
      .slice(from)
      .filter((token) => token.length >= 3 && !isAmbiguousAdvanceOpening(token) && unused(token));
    if (!distinctive.length) {
      return heardDistinct(recognized, verse, this.distinctSkip(verse, current));
    }
    return distinctive.some((token) => (
      recognized.some((word) => wordsMatch(word, token) || relatedStem(word, token) || softTokenMatch(word, token))
    ));
  }

  /** After a finished surah, lock ayah 1 of the newly located surah when that
   * opening is in the window — do not first-lock a later ayah such as 112:4. */
  private openingOfNewSurah(
    located: QuranVerse,
    text: string,
    recognized: string[],
    current: QuranVerse,
  ): QuranVerse | undefined {
    if (!this.preferSurahOpening || located.surah === current.surah) return undefined;
    const first = this.firstUsableOpening(located.surah);
    if (!first || this.sameRef(first, located)) return undefined;
    const after = this.queryAfterCurrent(recognized, current);
    return this.firstLockableOpening(located.surah, after.text, after.recognized)
      ?? this.firstLockableOpening(located.surah, text, recognized);
  }

  private firstUsableOpening(surah: number): QuranVerse | undefined {
    const first = this.db.getVerse(surah, 1);
    if (first && !skipUnusableLock(first)) return first;
    const second = this.db.getVerse(surah, 2);
    return second && !skipUnusableLock(second) ? second : undefined;
  }

  private firstLockableOpening(surah: number, text: string, recognized: string[]): QuranVerse | undefined {
    const verse = this.firstUsableOpening(surah);
    if (!verse || this.sameRef(verse, this.lock ?? undefined)) return undefined;
    if (!this.surahOpeningHeard(verse, recognized, text)) return undefined;
    if (this.lock && !this.nextSurahUnusedEvidence(verse, recognized, this.lock)) return undefined;
    const score = Math.max(this.locationScore(text, verse), LOCK_SCORE);
    const match: QuranChampionMatch = {
      surah: verse.surah,
      ayah: verse.ayah,
    ...
    if (this.ambiguousSurah(match, verse, text) || !this.canLock(match, verse, text, recognized)) return undefined;
    return verse;
  }

  /** After a finished surah, do not first-lock a later ayah (112:4) unless
   * ayah 1 is unusable or needs a later ayah to break a shared opening (113 vs 114). */
  private shouldHoldForSurahOpening(
    chosen: QuranVerse,
    current: QuranVerse,
    text: string,
    recognized: string[],
  ): boolean {
    if (!this.preferSurahOpening || chosen.surah === current.surah) return false;
    const first = this.firstUsableOpening(chosen.surah);
    if (!first || chosen.ayah <= first.ayah) return false;
    if (skipUnusableLock(first)) return false;
    if (this.firstLockableOpening(first.surah, text, recognized)) return false;
    const { basmala } = verseAlignWords(first);
    const sharedBody = this.uniqueOpeningSkip(first) - basmala;
    // Shared one-token opening (قل): first-locking 112:4 skips 112:1.
    // Shared two-plus (قل أعوذ برب): Falaq may start at 113:2 to break 114:1.
    // Do not use surah length — unit corpora often omit later ayahs (113 last=2).
    return sharedBody <= 1;
  }
